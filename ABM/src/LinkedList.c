...
float ll_findMaximo(LinkedList* this, float (*fn)(void* element))
{
	float maximo = 0;
	int i;
	float auxRetorno;
	...
	if(this != NULL)
	{
		...
		{
			auxiliar = ll_get(this,i);
			auxRetorno = fn(auxiliar);
			if(auxRetorno > maximo)
			{
				maximo = auxRetorno;
			}
		}
...
LinkedList* ll_filterPolaroid(LinkedList* this, int (*fn)(void* element))
{
	int i;
	int len;
	Revelado* auxiliar;
	...
	if(this != NULL && len > 0)
	{
		listaPolaroid = ll_newLinkedList();
		if (listaPolaroid != NULL)
		{
			for(i = 0; i < len; i++)
			{
				auxiliar = ll_get(this,i);
